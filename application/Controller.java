...
/**
 * This class represents the current social network of the user. It stores the
 * userNetwork data structure of the network, the current central user, and the
 * log file that can be exported by the user. This class contains methods to add
 * and remove users from the network, add and remove friendships between users,
 * etc. It is utilized by Main to implement the user's actions in
 * their social network.
 * 
 * @author Megan de Joya (dejoya@wisc.edu), Abhinav Kaushik
...
public class Controller {
	...
	static Graph userNetwork; // userNetwork representation of friend network
	private String centralUser; // current central user
	static File file; // log file to be exported

	/**
	 * default no-argument constructor initializes the userNetwork log file
	 */
	public Controller() {
	...
	/**
	 * creates a Controller object with a user-specified file name & initializes the
	 * userNetwork
	 * 
	...
	/**
	 * this method changes the central user and logs action in log file
	 * 
	...
	/**
	 * this method prints the friends of the specified user i.e. any user that this
	 * user (vertex) shares an edge with
	 * 
	...
	public void printCtrlNetwork(String user) {
		...
		Main.primaryStage.setScene(Main.Network(friends));
	}

	/**
	 * this method adds a friendship (edge on userNetwork) between two specified
	 * users and logs this action in the log file
	 * 
	 * @param user1
	 * @param user2
	...
	public boolean addFriend(String user1, String user2) {
		// since the implemented userNetwork is directed, an edge must be added both
		// ways
	...
	/**
	 * this method removes a friendship (edge on userNetwork) between two specified
	 * users and logs this action in the log file
	 * 
	 * @param user1
	 * @param user2
	...
	/**
	 * this method reads the contents of an input file and calls addFriend(),
	 * addUser(), removeFriend(), removeUser(), and setCentralUser() accordingly. if
	 * there is an error reading the file, this method displays an error message to
	 * the user
	 * 
		...
		try {
			...
			for (int i = 0; i < listCommands.size(); i++) {
				...
				if (command.length < 2 || command.length > 3) {
					primaryStage.setScene(Main.errorMessage(Main.uploadNetworkFile(), error));
					break;
				}
			...
			primaryStage.setScene(Main.errorMessage(Main.uploadNetworkFile(),
					"ERROR: The input file was not found"));
			return;
		}
		// display message upon success
		primaryStage.setScene(
				Main.successMessage(Main.uploadNetworkFile(), "SUCCESS: Network File Uploaded!"));
		return;
	}

	/**
	 * this method exports a .txt file that represents the network to a desired
	 * location on the user's computer
	 * 
	 * @return
	 */
	public static void exportFile(String filePath, boolean isExit) {
		try {
			// checks if the filepath is correctly formatted or not
			if (!filePath.contains("'\'")) {
				// checks what the users intentions were
				if (!isExit) {
					// display error message
					Main.primaryStage.setScene(Main.errorMessage(Main.ExportFile(),
							"ERROR: Invalid input for file path."));
				} else {
					// display error message
					Main.primaryStage.setScene(
							Main.errorMessage(Main.exit(), "ERROR: Invalid input for file path."));
				}
			}
			filePath.replace("'\'", "\\");
			Writer output = null;
			...
			// checks if the user intends to exit or not to change which location they are
			// directed to
			if (!isExit) {
				// display error message
				Main.primaryStage.setScene(
						Main.errorMessage(Main.ExportFile(), "ERROR: Could Not Write File."));
			} else {
				// display error message
				Main.primaryStage
						.setScene(Main.errorMessage(Main.exit(), "ERROR: Could Not Write File."));
			}
		}
		// checks if the user intends to exit or not to determine whether the
		// application exits or not.
		if (!isExit) {
			Main.primaryStage
					.setScene(Main.successMessage(Main.ExportFile(), "SUCCESS: File was written!"));
		} else {
			Platform.exit();
		}
	}
	...
	/**
	 * this method adds a user (vertex in userNetwork) to the social network and
	...
	/**
	 * this method removes a user (vertex in userNetwork) from the social network
	...
	/**
	 * this method returns a list of the mutual friends between two specified user
	 * i.e. any user (vertex) who is friends with (has an edge between) both users
	 * 
	 * @param user1
	 * @param user2
	...
	/**
	 * returns the number of groups (connected components) in the network
	 * 
	 * @return
	 */
	static int getGroups() {
		return userNetwork.connectedComponents();
	}
