...
/**
 * Filename: Graph.java Project: p4 Authors: Max Johnson Due Date: Nov. 14th
 * 
...
public class Graph implements GraphADT {
	...
	private static int degree; // the # of vertices
	private static List<GraphNode<String>> vertList; // List of all the vertices
	boolean[][] adjMatrix; // 2D list of each vertex represented by an index on each axis. the row
							// index
							// will tell where each edge comes from and the columns will tell where the
							// edge
							// goes to
	...
	public Graph() {
		...
		vertList = new ArrayList<GraphNode<String>>();// creates a new arrayList of GraphNodes of String
														// type
	...
	public boolean addVertex(String vertex) {
		...
		if (vertex == null) {
			return false;// ends the method if the input is null
		}
		// checks if the vertex exists or not already in the graph
		if (vertExist(vertex)) {
			return false; // ends the method if the vertex exists in the graph
		}
		...
		if (AMINDEX >= adjMatrix.length) {
			boolean[][] matrix = new boolean[AMINDEX * 2][AMINDEX * 2];// doubles the length of the
																		// current matrix
	...
	public boolean removeVertex(String vertex) {
		...
		// checks if the vertex exists or not already in the graph
		if (!vertExist(vertex)) {
			return false; // ends the method if the vertex doesnt exist in the graph
		}
	...
	public Set<String> getAllVertices() {
		...
		for (int i = 0; i < vertList.size(); i++) {
			// creates a temporary string from accessing the nodes vertex then adds it to
			// the set
	 ...
	 */
	private static GraphNode<String> findVertex(String vert) {
		// iterates through the list of vertices and compares the input vertex to the
		// vertex in iteration
		for (int i = 0; i < degree; i++) {
			if (vertList.get(i).getVertex().equals(vert)) {
	...
	private GraphNode<String> removeVertexHelper(String vert) {
		...
		for (int i = 0; i < degree; i++) {
			if (vertList.get(i).getVertex().equals(vert)) {
				GraphNode<String> node = vertList.remove(i);// creates a temporary variable for the node
															// removed
	...
	/**
	 * Returns the path from the startNodeName to the endNodeName
	 * 
	 * @param startNodeName - starting user
	 * @param endNodeName   - ending user
	 * @return List<String> - a list from the given node to the ending node
	 */
	static List<String> shortestPath(String startNodeName, String endNodeName) {
		// key node, value parent
		Map<String, String> parents = new HashMap<String, String>();
		List<GraphNode> temp = new ArrayList<GraphNode>();

		GraphNode start = findVertex(startNodeName);
		temp.add(start);
		parents.put(startNodeName, null);

		while (temp.size() > 0) {
			GraphNode currentNode = temp.get(0);
			List<GraphNode> neighbors = currentNode.getNeighbors();

			for (int i = 0; i < neighbors.size(); i++) {
				GraphNode neighbor = neighbors.get(i);
				String nodeName = (String) neighbor.getVertex();
				...
				if (visited) {
					...
					parents.put(nodeName, (String) currentNode.getVertex());
	...
	static List<String> getPath(Map<String, String> parents, String endNodeName) {
	...
	private void DFSHelper(int v, boolean[] visited) {
		...
		List<GraphNode<String>> adj = vertList.get(v).getNeighbors();
		for (int i = 0; i < adj.size(); i++) {
			if (!visited[adj.get(i).getIndex()]) {
				DFSHelper(adj.get(i).getIndex(), visited);
			}
		}
	}

	/**
	 * this method finds the number of connected components in the graph
	 * 
	 * @return
	 */
